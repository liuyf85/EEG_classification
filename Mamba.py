...
class EEGClassificationModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        ...
        config = MambaConfig(d_model=input_size, n_layers=num_layers, d_state = hidden_size)
    ...
    def forward(self, x):
        ...
        if is_training:
            noise = torch.normal(mean=0.0, std=0.1, size=x.size()).to(x.device)
            x = x + noise
...
def test(hidden_size,
    ...
    for fold in range(10):
        ...
        model = EEGClassificationModel(input_size, hidden_size, num_layers, num_classes).to(device)
    ...
    # for fold in range(10):
    #     print(f'fold = {fold}: Accuracy of the model on the test set: {accuracy_fold[fold]:.2f}%')
